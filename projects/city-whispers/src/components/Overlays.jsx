...
// ── Intro: video first (sound on by default), then the text letter ──
export function Intro({ open, onClose, onStartTour }) {
  ...
  useEffect(() => {
    if (!open || phase !== 'video' || reduced) return
    const v = videoRef.current
    if (!v) return
    v.currentTime = 0
    v.muted = false
    setMuted(false)
    // some browsers block autoplay-with-sound without prior interaction —
    // fall back to muted playback rather than leaving the video frozen
    v.play().catch(() => {
      v.muted = true
      setMuted(true)
      v.play().catch(() => {})
  ...
  if (open && phase === 'video' && !reduced) {
    return (
      <div id="intro-overlay" className="">
        <div id="intro-video-card">
          <video
            ref={videoRef}
            id="intro-video"
            src="/video/intro.mp4"
            ...
            playsInline
            controls
            onEnded={() => setPhase('letter')}
          />
        </div>
        <button className="intro-tour-link" onClick={() => setPhase('letter')}>Skip video</button>
      </div>
    )
  }
